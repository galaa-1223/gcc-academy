...
type PLProps = {
  ...
  menuGrid: string;
  title: string;
  gridNumber: number | string;
}

const PageLayouts = ({ MenuData, menuGrid, title, gridNumber }: PLProps) => {
  ...
  return (
    <div className="col-lg-12 col-xl-3 col-xxl-3 single-mega-item">
      ...
      <ul className="mega-menu-item">
        {MenuData &&
          MenuData.menuData.map((data: {menuType: string; gridMenuItems1: any; gridMenuItems2: any; gridMenuItems3: any}, index: number) => {
            if (data.menuType === menuGrid) {
              ...
              const elements = data.gridMenuItems3?.map((value: {link: string; coming: string; title: string; subTitle: string}, innerIndex: number) => (
                <li key={innerIndex}>
                  <Link
                    className={isActive(value.link) ? "active" : ""}
                    href={value.coming ? "/maintenance" : value.link}
